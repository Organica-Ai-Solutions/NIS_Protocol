    ...
    if training_file.exists():
        ...
        for ex in examples:
            training_data.append({
                ...
                "pipeline_score": ex.get("pipeline_score", 0.7),
...
def generate_synthetic_robotics_data(num_samples: int = 1000) -> Dict[str, Any]:
    ...
    for i in range(num_samples):
        ...
        if mission == "survey":
            ...
            output = f"Inspection mode activated. Reducing altitude to {max(10, abs(position[2]) - 30):.0f}m for detailed imAIng. Camera resolution: optimal. Stability: engaged."
        elif mission == "search_rescue":
            output = f"Search pattern initiated. Grid size: 100m x 100m. Thermal imAIng: active. Estimated search time: {np.random.uniform(10, 60):.0f} minutes."
        else:  # mapping
