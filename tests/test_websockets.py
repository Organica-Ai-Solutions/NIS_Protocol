...
async def test_consciousness_ws():
    """Test Consciousness WebSocket endpoint"""
    uri = "ws://localhost:8007/v4/consciousness/ws"
    try:
        async with websockets.connect(uri) as websocket:
            # Send test message
            await websocket.send(json.dumps({
                "operation": "genesis",
                "data": {"prompt": "Test consciousness"}
            }))
            
            # Receive response
            response = await asyncio.wait_for(websocket.recv(), timeout=5.0)
            data = json.loads(response)
            print(f"✅ Consciousness WebSocket: {data.get('type', 'unknown')}")
            return True
    except Exception as e:
        print(f"❌ Consciousness WebSocket: {e}")
        return False
