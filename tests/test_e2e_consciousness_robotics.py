...
@pytest.mark.asyncio
async def test_e2e():
    logger.info("🧠🤖 STARTING E2E pipeline -> ROBOTICS TEST")
    ...
    try:
        result = await pipeline.execute_autonomous_plan("goal_e2e_1", goal)
        ...
        if "drone" in steps_str or "survey" in steps_str:
            logger.info("✅ pipeline successfully incorporated intent into plan")
        else:
    ...
    logger.info("\n🧪 Step 2: Testing Direct Embodiment Action (pipeline -> Physics)")
    # This tests the path: Endpoint -> pipelineService -> Embodiment -> Physics Check
    
    action_type = "move_drone"
    params = {"x": 10, "y": 20, "z": 5}
    
    try:
        action_result = await pipeline.execute_embodied_action(action_type, params)
